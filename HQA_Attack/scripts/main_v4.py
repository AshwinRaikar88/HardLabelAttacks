...
def main(dataset_name, synonym_method, start_idx):
    if not HF_TOKEN:
    ...
    for idx, sample in enumerate(samples.select(range(start_idx-1, len(samples))) , start=start_idx):
        print(f"\n{'='*70}")
...
if __name__ == "__main__":    
    ...
    main(args.dataset, args.synonym_method, args.start_idx)
