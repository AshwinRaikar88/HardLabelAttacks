...
class Qwen3ClassifierLogits:
    """Qwen3 classifier using logits-based prediction"""
    
    def __init__(self, model_path, base_model="unsloth/Qwen3-1.7B", 
        ...
        print(f"Qwen3 classifier loaded for {dataset} dataset!\n")
    
    def _format_prompt(self, text):
        """Format prompt based on dataset type - CRITICAL: matches training format"""
        if self.task_type == 'sentiment':
            prompt = f"Label the following movie review as one of the following labels: positive, negative.\n\nReview: {text}"
        else:  # topic classification
            labels_str = ", ".join([l.lower() for l in self.labels])
            prompt = f"Label the following text as one of the following categories: {labels_str}.\n\nText: {text}"
        
        messages = [{"role": "user", "content": prompt}]
        
        # CRITICAL: Must use enable_thinking=False to match training
...
class HQAAttackQwen3Logits:
    """HQA-Attack using logits-based prediction for Qwen3"""
    ...
    def __init__(self, model_path,
        ...
        # Initialize Qwen3 classifier with logits
        self.classifier = Qwen3ClassifierLogits(
        ...
        print(f"HQA Attack (Logits) initialized for Qwen3 on {dataset}")
...
def save_checkpoint(checkpoint_file, results, start_idx, total_samples, 
                   success_count, dataset, total_time, sample_times, batch_start=None, batch_end=None):
    ...
    checkpoint = {
        ...
        'success_rate': success_count / (start_idx + 1) if start_idx >= 0 else 0,
...
def main():
    parser = argparse.ArgumentParser(description="HQA Attack on Qwen3 using Logits")
    parser.add_argument("--model_path", type=str, required=True, help="Path to fine-tuned model")
    parser.add_argument("--base_model", type=str, default="unsloth/Qwen3-8B")
    ...
    parser.add_argument("--output_file", type=str, default="hqa_attack_qwen3_logits.json")
    ...
    # Updated to use the Mistral-style dataset loading logic
    ...
    batch_end = args.end_idx  # Will be set after loading dataset
    ...
    print(f"Processing {len(test_data)} samples from this batch")

    # Set checkpoint and output files with batch suffix (Mistral logic)
    ...
    if checkpoint:
        ...
        print(f"Previous progress: {success_count} successful attacks")
    ...
    print(f"\n{'='*80}")
    print(f"STARTING ATTACK CAMPAIGN")
    print(f"{'='*80}")
    print(f"Model: Qwen3")
    ...
    for i in range(start_idx, len(test_data)):
        ...
        print(f"\n{'='*80}")
        print(f"Batch Sample {i+1}/{len(test_data)} | Dataset Index: {actual_idx}")
        ...
        try:
            result = attacker.attack(text, max_iterations=args.max_iterations, verbose=True)
            result['dataset_index'] = actual_idx
            ...
            print(f"\n{'─'*80}")
            print(f"Sample {i+1} (Dataset idx: {actual_idx}) completed in {sample_time:.2f}s")
            ...
            print(f"\n[ERROR] Failed on sample {i+1} (dataset idx: {actual_idx}): {str(e)}")
            current_time = time.time() - total_start_time + previous_time
            save_checkpoint(
                args.checkpoint_file, 
                results, 
    ...
    print(f"Success rate: {success_count / len(test_data) * 100:.2f}%")
    print(f"{'='*80}")
